...
export async function scrapeProjects() {
    ...
    // 🚀 Try Axios up to 2 times with a generous 25-second timeout
    for (let attempt = 1; attempt <= 2; attempt++) {
        try {
            logger.info(`Attempting fast scrape with Axios (Attempt ${attempt}/2)...`);
            ...
            const { data } = await axios.get(cacheBusterUrl, {
                ...
                timeout: 25000 // 25 seconds (prevents premature timeouts on slow network spikes)
            });

            const projects = parseHtml(data);
            if (projects.length > 0) {
                return projects; // Success! Return immediately
            }

            logger.warn(`Axios Attempt ${attempt} returned 0 projects.`);
        } catch (error) {
            logger.warn(`Axios Attempt ${attempt} failed (${error.message}).`);
        }

        // Brief 2-second pause before retry
        if (attempt < 2) {
            await new Promise(res => setTimeout(res, 2000));
        }
    }

    // Only run Playwright as an absolute last resort if all Axios attempts fail
    logger.warn('All Axios attempts failed. Trying Playwright fallback as last resort...');
...
async function scrapeWithPlaywright() {
    ...
    try {
        ...
        await page.goto(config.projectUrl, { waitUntil: 'domcontentloaded', timeout: 35000 });
        await page.waitForSelector('.project-row, table tbody tr', { timeout: 15000 }).catch(() => {});
